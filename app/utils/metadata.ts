...
function displayTitleFromMdxPath(mdxPath: string[] | undefined): string {
  ...
  if (parts.length === 0) return "El Pantano Docs";
  const first = parts[0];
  if (parts.length === 1 && DOC_LOCALES.has(first)) {
    return "El Pantano Docs";
  }
  if (DOC_LOCALES.has(first)) {
    return parts.slice(1).join(" ") || "El Pantano Docs";
  }
  return parts.join(" ") || "El Pantano Docs";
}
...
export async function generateMetadata({ params }: { params: { mdxPath?: string[] } }) {
  ...
  const url = `https://docs.elpantano.org/${urlPath}`;
  const image = `https://docs.elpantano.org/api/og?title=${encodeURIComponent(title)}`;
  ...
  let description = "Documentación del Pantano.";
